...
/*	@Query(value="select a.id,a.erhDistrubutor,b.matCode,b.matDesc,b.apprQty,b.leftOverQty,b.id from EzcRequestHeader a,RequestMaterials b where a.id=b.ezcRequestHeader.id and a.erhStatus=:erhStatus and a.erhReqType=:erhReqType")
	...
	@Query(nativeQuery = true,value="SELECT Month(erh_requested_on) as MON , COUNT(*) ,ERH_REQ_TYPE,erh_requested_by,SUM(CASE WHEN ERH_STATUS='SUBMITTED' THEN 1 ELSE 0 END) AS SUBMITTED FROM ezc_request_header where year(ERH_REQUESTED_ON)=:selYear GROUP BY MONTH(erh_requested_on),ERH_REQ_TYPE,erh_requested_by")
	List<Object[]> getAllReqMonthWise(Integer selYear);
	...
	@Query(nativeQuery = true,value="select COUNT(eri_plumber_name),(SELECT erh_requested_by from ezc_request_header where id=eri_req_id) emp,ERH_REQ_TYPE from ezc_request_items a,ezc_request_header b where a.eri_req_id=b.id and year(b.ERH_REQUESTED_ON)=:selYear and b.erh_status='SUBMITTED' and b.ERH_REQ_TYPE!='BD' group by eri_req_id")
	List<Object[]> getNoofPlumbersPerUser(Integer selYear);
	...
	@Query(nativeQuery=true,value="select erh_status,SUM(erm_appr_qty), SUM(erm_leftover_qty),(SELECT erh_requested_by from ezc_request_header where id=erm_req_id) emp,ERH_REQ_TYPE from ezc_request_materials a,ezc_request_header b where a.erm_req_id=b.id and year(b.ERH_REQUESTED_ON)=:selYear and b.erh_status in('SUBMITTED','APPROVED') and b.ERH_REQ_TYPE!='BD' group by erm_req_id,erh_status")
	List<Object[]> getUsedLeftQtyPerUser(Integer selYear);
	@Query(nativeQuery=true,value="SELECT  COUNT(*) ,ERH_REQ_TYPE,erh_requested_by,SUM(CASE WHEN ERH_STATUS!='SUBMITTED' THEN 1 ELSE 0 END) AS INPROCESS FROM ezc_request_header WHERE year(ERH_REQUESTED_ON)=:selYear and erh_requested_on >= NOW() - INTERVAL 1 YEAR GROUP BY ERH_REQ_TYPE,erh_requested_by")
	List<Object[]> getAllInProcessReqPerUser(Integer selYear);
	...
	@Query(nativeQuery = true,value="SELECT A.ERH_REQUESTED_ON,A.ERH_REQ_TYPE,A.ID,A.ERH_DISTRIBUTOR,A.ERH_DIST_NAME,A.ERH_REQUESTED_BY,A.ERH_REQ_NAME,A.ERH_CITY,A.ERH_COST_INCURED,A.ERH_VERTICAL,(SELECT COUNT(ID) FROM EZC_REQUEST_ITEMS WHERE ERI_REQ_ID=A.ID) AS ATTENDES,(SELECT CONCAT(CAST(SUM(ERM_APPR_QTY) AS CHAR(20)),'#',CAST(SUM(ERM_LEFTOVER_QTY) AS CHAR(20))) FROM EZC_REQUEST_MATERIALS WHERE ERM_REQ_ID=A.ID) AS APPR_QTY,(SELECT GROUP_CONCAT(ERD_DEALER_NAME SEPARATOR ',') FROM EZC_REQUEST_DEALERS WHERE ERD_REQ_ID=A.ID) AS RETAILER,A.ERH_STATUS FROM EZC_REQUEST_HEADER A WHERE  A.ERH_REQ_TYPE='TPS' AND (A.ERH_REQUESTED_ON >= :fromDate AND A.ERH_REQUESTED_ON <= :toDate)")
	List<Object[]> getTPSSummary(Date fromDate,Date toDate);
	@Query(nativeQuery = true,value="SELECT A.ERH_REQUESTED_ON,A.ERH_REQ_TYPE,A.ID,A.ERH_DISTRIBUTOR,A.ERH_DIST_NAME,A.ERH_REQUESTED_BY,A.ERH_REQ_NAME,A.ERH_CITY,A.ERH_COST_INCURED,A.ERH_VERTICAL,(SELECT COUNT(ID) FROM EZC_REQUEST_ITEMS WHERE ERI_REQ_ID=A.ID) AS ATTENDES,(SELECT CONCAT(CAST(SUM(ERM_APPR_QTY) AS CHAR(20)),'#',CAST(SUM(ERM_LEFTOVER_QTY) AS CHAR(20))) FROM EZC_REQUEST_MATERIALS WHERE ERM_REQ_ID=A.ID) AS APPR_QTY,(SELECT GROUP_CONCAT(ERD_DEALER_NAME SEPARATOR ',') FROM EZC_REQUEST_DEALERS WHERE ERD_REQ_ID=A.ID) AS RETAILER,(SELECT SUM(ERD_COST_INCURED) FROM EZC_REQUEST_DEALERS WHERE ERD_REQ_ID=A.ID) AS COST_INCURED,A.ERH_STATUS FROM EZC_REQUEST_HEADER A WHERE  A.ERH_REQ_TYPE='TPM' AND (A.ERH_REQUESTED_ON >= :fromDate AND A.ERH_REQUESTED_ON <= :toDate)")
	List<Object[]> getTPMSummary(Date fromDate,Date toDate);
	@Query(nativeQuery = true,value="SELECT A.ERH_REQUESTED_ON,A.ERH_REQ_TYPE,A.ID,A.ERH_CITY,A.ERH_REQUESTED_BY,A.ERH_REQ_NAME,A.ERH_DISTRIBUTOR,A.ERH_DIST_NAME,A.ERH_PURPOSE,B.ERM_MAT_CODE,B.ERM_MAT_DESC,B.ERM_APPR_QTY,B.ERM_APPR_QTY-B.ERM_USED_QTY,A.ERH_STATUS FROM EZC_REQUEST_HEADER A,EZC_REQUEST_MATERIALS B WHERE A.ID=B.ERM_REQ_ID AND A.ERH_REQ_TYPE='BD'  AND (A.ERH_REQUESTED_ON >= :fromDate AND A.ERH_REQUESTED_ON <= :toDate)")
	...
	@Query(nativeQuery = true,value="SELECT A.ERH_REQUESTED_ON,A.ERH_REQ_TYPE,A.ID,A.ERH_DISTRIBUTOR,A.ERH_DIST_NAME,A.ERH_REQUESTED_BY,A.ERH_REQ_NAME,A.ERH_CITY,A.ERH_COST_INCURED,A.ERH_VERTICAL,(SELECT COUNT(ID) FROM EZC_REQUEST_ITEMS WHERE ERI_REQ_ID=A.ID AND ERI_DEALER_ID=B.ID) AS ATTENDES,B.ERD_MEET_ID,B.ERD_MEET_DATE,B.ERD_DEALER_NAME,B.ERD_COST_INCURED FROM EZC_REQUEST_HEADER A,EZC_REQUEST_DEALERS B WHERE A.ID=B.ERD_REQ_ID AND  A.ERH_REQ_TYPE='TPM' AND (A.ERH_REQUESTED_ON >= :fromDate AND A.ERH_REQUESTED_ON <= :toDate)")
	List<Object[]> getTPMMeetSum(Date fromDate, Date toDate);
	@Query(nativeQuery = true,value="SELECT A.ERH_REQUESTED_ON,A.ERH_REQ_TYPE,A.ID, A.ERH_REQUESTED_BY,A.ERH_REQ_NAME,A.ERH_DISTRIBUTOR,A.ERH_DIST_NAME,B.ERM_MAT_CODE,B.ERM_MAT_DESC,B.ERM_APPR_QTY FROM EZC_REQUEST_HEADER A,EZC_REQUEST_MATERIALS B WHERE A.ID=B.ERM_REQ_ID AND A.ERH_DISPATCH_FLAG='Y' AND (A.ERH_REQUESTED_ON >= :fromDate AND A.ERH_REQUESTED_ON <= :toDate)")
	List<Object[]> getDispatchedGifts(Date fromDate, Date toDate);		
